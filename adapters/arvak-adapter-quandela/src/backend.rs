...
/// this is a contract test boundary (see `test_internalcode_enrollment_schema_matches_arvak_struct`).
...
pub struct InternalCodeEnrollment {
    ...
    /// regardless of this field — internalcode analyses the *compressor drive*, not
...
///
/// InternalCode integration is complete: use `ingest_internalcode_enrollment` to populate
/// the PUF enrollment from internalcode-lab output, and `ingest_internalcode_schedule` to
...
impl QuandelaBackend {
    ...
    /// Bridge: ingest an internalcode-lab enrollment record into the `CoolingProfile`.
    ///
    ...
    /// Bridge: ingest quiet windows from internalcode-lab scheduling output.
    ///
